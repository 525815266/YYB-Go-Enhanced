...
- 新增 `scripts/毛毛充.py`：基于毛毛充 HAR 的动态登录、积分查询、每日签到和视频任务；视频默认最多三次并按服务端次数实时限流，未抓到的积分抽奖接口不做猜测调用。
- 修复 `毛毛充.py` 解析 YYB `/wxapp/getCode` 响应错误：兼容当前 `data.result.code`、旧版嵌套结构和直接返回结构，避免有效 code 被误报为空；取码失败时输出脱敏诊断信息。

## 2026-09-08

- 新增“已激活短链接”管理页：普通用户按归属隔离，管理员可查看全部链接，并支持按状态/类型筛选、复制、打开、作废和删除。
- 授权链接历史记录不再在生成时自动清理；管理页保留创建、失效、消费、作废状态。新建更新/新增链接时会返回同范围内未消费链接，便于先作废旧链接或继续生成。
