...
- 新增 `scripts/boluo.py`“菠萝充电-浪潮” YYB 脚本：按账号动态获取微信 code，查询会员昵称/ID/积分，每日签到一次，视频任务默认最多三次并按服务端次数实时限流；签到和视频结果均会重新查询确认。
- 记录当前 HAR 尚未包含积分抽奖接口，脚本不会猜测或发送未知请求，日志会明确提示待补抓包后接入。
