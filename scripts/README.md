...
- `boluo.py`：菠萝充电-浪潮 YYB 版。通过 HAR 中确认的微信小程序 AppID
  `wxc7548b3f7181e9d9`（可用 `BOLUO_WECHAT_APP_ID` 覆盖）动态获取微信 code；业务
  请求 Header 使用 `hichar.user.wxapp`，两者不要混用。
  查询会员昵称/ID/积分，每日签到一次；视频任务默认执行 3 次，并在每次执行前
  重新读取服务端 `nowTimes/limitTimes`，不会超过当日上限。当前 HAR 未包含积分
  抽奖请求，因此脚本只展示积分并明确提示抽奖暂未接入，待补充抽奖页面抓包后再实现。
