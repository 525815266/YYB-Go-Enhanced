...
!(async () => {
    const plan = APPS.map((app) => ({
        app,
        accounts: filterAccounts(getAccounts(app), refOf, { appId: app.appid }),
    ...
    for (const { app, accounts } of plan) {
        ...
        for (const account of accounts) {
            const task = new Task(app, account, index++);
            const summary = await task.run();
            if (/未绑定此小程序|未授权手机号|尚未注册/.test(String(summary.sign || ""))) {
                markFromError(task.ref, summary.sign, app.appid);
            } else if (!/失败|未执行|获取code/.test(String(summary.sign || ""))) {
                setStatus(task.ref, "ready", "", app.appid);
            }
